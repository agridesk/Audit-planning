...
function AMS01_RunPromotionCheck() {
  ...
  probe_('Auditor perf override status', function(){
    return (typeof AMS01_AuditorPerfOverrideStatus === 'function') ? AMS01_AuditorPerfOverrideStatus() : { success:false, active:false };
  });
  probe_('Toolkit availability route status', function(){
    return (typeof AMS01_GetAvailabilityRouteStatus === 'function') ? AMS01_GetAvailabilityRouteStatus() : { success:false, active:false };
  });
  probe_('Status notification performance route status', function(){
    return (typeof AMS01_StatusNotificationPerfStatus === 'function') ? AMS01_StatusNotificationPerfStatus() : { success:false, active:false };
  });
  probe_('Status read performance route status', function(){
    return (typeof AMS01_StatusReadPerfStatus === 'function') ? AMS01_StatusReadPerfStatus() : { success:false, active:false };
  });
  probe_('Status core performance override status', function(){
    return (typeof AMS01_StatusActionCorePerfStatus === 'function') ? AMS01_StatusActionCorePerfStatus() : { success:false, active:false };
  });

  probe_('Promoted manager Toolkit bundle', function(){
    return getToolkitOpenBundleV5_d13(auditId, monthKey, { role:'MANAGER', withCalendar:false, forceFresh:true });
  });
  probe_('Qualification-only candidate', function(){ return AMS01_GetQualifiedAuditorsFastCandidate(auditId); });
  probe_('Promoted Toolkit visible-month availability', function(){
    return getToolkitAvailabilityMonthDirectV5(auditorEmail, monthKey, { forceFresh:true, bypassCache:true, failOnOverlayError:false });
  });
  ...
  probe_('Status action component benchmark', function(){
    return (typeof AMS01_RunStatusActionComponentBenchmark === 'function') ? AMS01_RunStatusActionComponentBenchmark() : { success:false };
  });
  ...
  var result = { build:out.build, generatedAt:out.generatedAt, runtimeEnv:out.runtimeEnv, probes:compact };
