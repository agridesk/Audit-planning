...
function RUN_AMS03_PLANNING2_MASTER_ACCEPTANCE(){
 ...
 var failed=results.filter(function(x){return!x.ok}).length,out={ok:failed===0,build:'2026-09-23_AMS03_PLANNING2_MASTER_GATE_R1',total:results.length,passed:results.length-failed,failed:failed,results:results,meta:{nonDestructive:true,newSsot:false,roadmap:'Planning 2.0 phases 2-10',phase1CapacityCorrection:'previously closed',phase8to10Status:'MATURATION_FOUNDATION_NOT_FULL_PRODUCT_CLOSURE',legacyRetirementPolicy:'safe caller migration only'}};Logger.log(JSON.stringify(out,null,2));return out;
}
