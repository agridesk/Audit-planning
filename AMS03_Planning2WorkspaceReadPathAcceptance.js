...
// BUILD: 2026-09-24_AMS03_PLANNING2_WORKSPACE_READ_PATH_R5
function RUN_AMS03_PLANNING2_WORKSPACE_READ_PATH_ACCEPTANCE(){
 ...
 g('eligibilityTelemetryUsesReadPack',!!(ec&&ec.meta&&ec.meta.readStrategy)&&(ec&&ec.meta&&ec.meta.windowFallback===true||ec&&ec.meta&&ec.meta.windowFallback===false),'Eligibility runtime result exposes actual bounded read strategy and fallback state.');
 ...
 var f=r.filter(function(x){return!x.ok}),out={ok:!f.length,build:'2026-09-24_AMS03_PLANNING2_WORKSPACE_READ_PATH_R5',passed:r.length-f.length,total:r.length,results:r,meta:{priority:'P0_SPEED',runtimeMeasurementStillRequired:true}};Logger.log(JSON.stringify(out,null,2));return out;
}
