  ...
  try{
    ...
    plan.actions.forEach(function(a){
      ...
      if(!obligation){
        ...
        obligation.Base_Expiry_Date='';obligation.Effective_Expiry_Date='';obligation.Extension_Applied='';obligation.Extension_Metadata_JSON='';
    ...
    var out={success:true,build:MODEL_C_ECAS_ANNUAL_IMPORT_BUILD,batchYear:plan.batchYear,writesPerformed:true,batchId:batchId,counts:{sourceRows:plan.counts.sourceRows,sourcePhysicalRows:plan.counts.sourcePhysicalRows,ignoredOtherServiceRows:plan.counts.ignoredOtherServiceRows,createdCompanyScopes:createdScopes,reactivatedCompanyScopes:reactivatedScopes,createdObligations:createdObligations,updatedHours:updatedHours,unchanged:unchanged,skippedCompleted:skippedCompleted,preservedPlanningWindows:preservedWindows,staleCanonicalNotInSource:verify.counts.staleCanonicalNotInSource||0},gates:{canonicalTargetsOnly:true,directAuditPlanningWrite:false,certificateLifecycleLeak:false,planningWindowsNotInvented:true,scopeAwareCompletionSuppression:true,postApplyReconciled:true},postVerify:ModelCEcasAnnualImport_compact_(verify),errors:[]};
...
function ModelCEcasAnnualImport_buildPlan_(ss){
  ...
  [MODEL_C_ECAS_SOURCE_SHEET,'Companies',MODEL_C_SHEETS.COMPANY_SCOPES,MODEL_C_SHEETS.AUDIT_OBLIGATIONS,MODEL_C_SHEETS.VISIT_OBLIGATIONS].forEach(function(n){if(!ss.getSheetByName(n))out.errors.push('Missing sheet: '+n);});
  ...
  if(ixMps<0)out.errors.push('Source missing header: MPS-nummer');if(ixHours<0)out.errors.push('Source missing header: Mandated time');if(ixServices<0)out.errors.push('Source missing header: Services');
  ...
  Object.keys(abcSourceByMps).sort().forEach(function(mps){
    var sourceItem=abcSourceByMps[mps],hours=sourceItem.mandatedHours,company=companyIndex[mps];
    if(!company){out.errors.push('MPS-nummer not found in Companies.Number: '+mps);return;}
    out.counts.matchedCompanies++;var uid=company.companyUid;sourceCompanyUids[uid]=sourceItem;
    if(completion.byCompanyUid[uid]){out.counts.completedAlready++;out.actions.push(ModelCEcasAnnualImport_action_('SKIP_COMPLETED',sourceItem,company,null,null,['PROVEN_MPS_ABC_COMPLETED']));return;}
    if(completion.genericByCompanyUid[uid])out.counts.genericCompletedIgnored++;

    var scope=csByCompany[uid]||null,action=ModelCEcasAnnualImport_action_('UPSERT',sourceItem,company,scope,null,[]);
...
function ModelCEcasAnnualImport_completedIndexScopeAware_(ss,sheet,year,obligations,links){
  ...
  for(var r=1;r<v.length;r++){
    ...
    var y=ixYear>=0?String(v[r][ixYear]||'').trim().slice(0,4):'';if(!/^20\d{2}$/.test(y)&&ixCompleted>=0)y=ModelCEcasAnnualImport_yearFromValue_(v[r][ixCompleted],ss);if(!/^20\d{2}$/.test(y)&&ixPlanned>=0)y=ModelCEcasAnnualImport_yearFromValue_(v[r][ixPlanned],ss);if(y!==String(year))continue;
...
function ModelCEcasAnnualImport_action_(kind,sourceItem,company,scope,obligation,reasons){return{action:kind,mpsNumber:sourceItem.mpsNumber,companyUid:company.companyUid,company:company.companyName||'',mandatedHours:sourceItem.mandatedHours,sourceRow:sourceItem.sourceRow,cycleYear:sourceItem.cycleYear,companyScopeId:scope?String(scope.Company_Scope_ID||''):'',obligationId:obligation?String(obligation.Obligation_ID||''):'',reasons:(reasons||[]).slice()};}
function ModelCEcasAnnualImport_companyIndex_(sheet,errors){var v=sheet.getDataRange().getValues(),h=v[0]||[],m=ModelCFoundation_headerMap_(h),ixNumber=ModelCEcasAnnualImport_header_(m,['Number','MPS-nummer','MPS nummer']),ixUid=ModelCEcasAnnualImport_header_(m,['Company_UID','Company UID']),ixName=ModelCEcasAnnualImport_header_(m,['Company','Company name','Name']);if(ixNumber<0||ixUid<0){errors.push('Companies missing Number or Company_UID');return{};}var out={};for(var r=1;r<v.length;r++){var number=String(v[r][ixNumber]||'').trim(),uid=String(v[r][ixUid]||'').trim();if(!number)continue;if(out[number]&&out[number].companyUid!==uid)errors.push('Duplicate Companies.Number: '+number);out[number]={mpsNumber:number,companyUid:uid,companyName:ixName>=0?String(v[r][ixName]||'').trim():'',row:r+1};}return out;}
