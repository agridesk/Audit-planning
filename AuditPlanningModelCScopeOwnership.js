...
function RUN_MODEL_C_PHASE2B_SCOPE_OWNER_PREFLIGHT() {
  ...
  var errors=[],cs=[],ob=[],lk=[];
  if(!csSheet||!obSheet||!lkSheet)errors.push('Missing Model C scope-owner sheet');
  if(!errors.length){
    cs=ModelCMigration_rowsToObjects_(csSheet.getDataRange().getValues());
    ob=ModelCMigration_rowsToObjects_(obSheet.getDataRange().getValues());
    lk=ModelCMigration_rowsToObjects_(lkSheet.getDataRange().getValues());
  }
  var csById={},obById={},activeLinks=0;
  ...
  var out={success:errors.length===0,readyForScopeOwnerRouting:errors.length===0,build:MODEL_C_SCOPE_OWNER_BUILD,readOnly:true,writesPerformed:false,counts:{companyScopes:cs.length,obligations:ob.length,activeLinks:activeLinks},policy:{deselect:'DEACTIVATE_CANCEL_UNLINK',historyDeleted:false,lifecycleOwner:'Config_Scopes.Recurring',planningWindowOwner:'ALL_ACTIVE_OBLIGATIONS',gapGraspSharedExpiry:true,currentAuditObligationRouting:true,legacyDateProjectionAsText:true,uiHoursReadFromModelC:true,safeSnapshotIntegrated:true,targetedLegacyWrites:true},errors:errors.slice(0,25)};
...
function ModelCScopeOwner_indexCurrentObligations_(companyUid,auditId,obligations,links){
  ...
  (links||[]).forEach(function(link){
    ...
    if(!obligation||String(obligation.Company_UID)!==companyUid)return;
  ...
  (obligations||[]).forEach(function(obligation){
    ...
    else if(String(byCs[csId].Obligation_ID)!==String(obligation.Obligation_ID))errors.push('Multiple current obligations for audit/company scope: '+auditId+'|'+csId);
  });
...
function ModelCScopeOwner_planTransition_(command,companyScopes,obligations,links){
  command=command||{};
  var companyUid=String(command.companyUid||''),auditId=String(command.auditId||''),selected=ModelCScopeOwner_normalizeSelected_(command.selectedScopes||[]),errors=[];
...
function ModelCScopeOwner_enrichUiScopes_(ss,companyUid,auditId,scopes){
  ...
  var csSheet=ss.getSheetByName(MODEL_C_SHEETS.COMPANY_SCOPES),obSheet=ss.getSheetByName(MODEL_C_SHEETS.AUDIT_OBLIGATIONS),lkSheet=ss.getSheetByName(MODEL_C_SHEETS.VISIT_OBLIGATIONS);
  var cs=csSheet?ModelCMigration_rowsToObjects_(csSheet.getDataRange().getValues()):[],ob=obSheet?ModelCMigration_rowsToObjects_(obSheet.getDataRange().getValues()):[],lk=lkSheet?ModelCMigration_rowsToObjects_(lkSheet.getDataRange().getValues()):[];
  ...
  return(scopes||[]).map(function(row){
    ...
    out.recurring=recurring;out.certificateBirthday=recurring&&match?ModelCFoundation_clean_(match.cs.Certificate_Birthday):'';out.baseExpiry=recurring&&match&&match.ob?ModelCExtension_dateInTz_(match.ob.Base_Expiry_Date,ss.getSpreadsheetTimeZone()):'';out.lifecycleType=recurring?'CERTIFICATE_RECURRING':'NON_RECURRING';
...
function ModelCScopeOwner_commit(command){
  command=command||{};var ss=SpreadsheetApp.getActive(),lock=LockService.getScriptLock();lock.waitLock(15000);var snapshots=[];
  try{
    ...
    Object.keys(csByCode).forEach(function(code){if(selected[code]||String(csByCode[code].Active).toUpperCase()!=='YES')return;var scope=csByCode[code],obligation=obByCs[String(scope.Company_Scope_ID)];scope.Active='NO';scope.Updated_At=stamp;if(obligation){obligation.Obligation_State='CANCELLED';obligation.Closed_At=stamp;obligation.Updated_At=stamp;var link=activeLinkByOb[String(obligation.Obligation_ID)];if(link){link.Link_State='INACTIVE';link.Unlinked_At=stamp;}}});
    ...
    var projection=ModelCScopeOwner_projectLegacy_(apSheet,command,selected,ob,lk,ss);SpreadsheetApp.flush();
...
function ModelCScopeOwner_triggerSource_(command,item){var explicit=String((command&&command.triggerSource)||'').trim();if(explicit)return explicit;return item&&item.recurring===true?'CERTIFICATE_LIFECYCLE':'MANUAL';}

function ModelCScopeOwner_newObligation_(command,scope,item,stamp,ss){return{Obligation_ID:ModelCMigration_newId_('OBL_'),Company_Scope_ID:scope.Company_Scope_ID,Company_UID:String(command.companyUid),ScopeCode:item.scopeCode,Cycle_Key:'',Trigger_Source:ModelCScopeOwner_triggerSource_(command,item),Obligation_State:'OPEN',Base_Expiry_Date:'',Extension_Applied:'',Extension_Metadata_JSON:'',Effective_Expiry_Date:'',Planning_Window_From:'',Planning_Window_To:'',Formal_Hours:ModelCScopeOwner_hours_(item.formalHours),Preassigned_Auditor_Email:'',Allow_Self_Planning:'',Migration_Batch_ID:'',Source_Audit_ID:String(command.auditId),Created_At:stamp,Updated_At:stamp,Closed_At:''};}
...
function ModelCScopeOwner_updateObligation_(obligation,command,item,stamp,ss){
  ...
  if(item.recurring!==true){obligation.Base_Expiry_Date='';obligation.Effective_Expiry_Date='';obligation.Extension_Applied='';obligation.Extension_Metadata_JSON='';return;}
...
function ModelCScopeOwner_window_(scopeCode,expiry,tz){var cfg=ModelCRecurringConfig_get_(SpreadsheetApp.getActive(),scopeCode),parts=String(expiry||'').split('-'),date=new Date(Number(parts[0]),Number(parts[1])-1,Number(parts[2]));if(isNaN(date.getTime()))throw new Error('Invalid expiry date for '+scopeCode);var from=Utilities.formatDate(AC_addMonthsPreserveDay_(date,Number(cfg.planningFrom||0)),tz,'yyyy-MM-dd'),to=Utilities.formatDate(AC_addMonthsPreserveDay_(date,Number(cfg.planningTo||0)),tz,'yyyy-MM-dd');if(from>to){var swap=from;from=to;to=swap;}return{from:from,to:to};}
...
function ModelCScopeOwner_projectLegacy_(sheet,command,selected,obligations,links,ss){
  ...
  var original=values[rowIndex-1].slice(),row=original.slice(),defs=m5t_scopeSlotDefs_(ss),total=0,linked={},active=[];
  ...
  obligations.forEach(function(x){if(linked[String(x.Obligation_ID)]&&['CANCELLED','REJECTED','COMPLETED'].indexOf(String(x.Obligation_State||'').toUpperCase())<0)active.push(x);});
  ...
  cert.forEach(function(x){var e=ModelCExtension_dateInTz_(x.Base_Expiry_Date,ss.getSpreadsheetTimeZone()),z=ModelCExtension_dateInTz_(x.Effective_Expiry_Date,ss.getSpreadsheetTimeZone());if(e&&(!earliest||e<earliest))earliest=e;if(z&&(!effectiveEarliest||z<effectiveEarliest))effectiveEarliest=z;if(String(x.Extension_Applied||'').trim())extension='Yes';var it=selected[String(x.ScopeCode)];if(it&&it.certificateBirthday&&!birthday)birthday=it.certificateBirthday;});
  active.forEach(function(x){var f=ModelCExtension_dateInTz_(x.Planning_Window_From,ss.getSpreadsheetTimeZone()),t=ModelCExtension_dateInTz_(x.Planning_Window_To,ss.getSpreadsheetTimeZone());if(f&&(!from||f>from))from=f;if(t&&(!to||t<to))to=t;});
  ...
  var textCols={};['Birthdate certificate','Date - Will Expire','Extended Expiration Date','Planning window from','Planning window to'].forEach(function(h){var c=map[ModelCFoundation_normHeader_(h)];if(c!==undefined)textCols[c]=true;});
  var touched=0;
  for(var c=0;c<row.length;c++){
    var a=original[c],b=row[c],same=(a===b)||(String(a==null?'':a)===String(b==null?'':b));
    if(same)continue;
    var cell=sheet.getRange(rowIndex,c+1);if(textCols[c])cell.setNumberFormat('@');cell.setValue(b===undefined?'':b);touched++;
  }
  return{rowIndex:rowIndex,totalHours:total,recurringScopeCount:cert.length,planningWindowScopeCount:active.length,planningWindowFrom:from,planningWindowTo:to,compatibilityExpiry:earliest,targetedCellsWritten:touched};
}
...
function ModelCScopeOwner_snapshotSheet_(sheet){
  ...
  if(String(sheet.getName())===String(MODEL_C_SHEETS.AUDIT_PLANNING)&&values.length){var headers=values[0]||[],map=ModelCFoundation_headerMap_(headers),tz=sheet.getParent().getSpreadsheetTimeZone()||Session.getScriptTimeZone();['Birthdate certificate','Date - Will Expire','Extended Expiration Date','Planning window from','Planning window to'].forEach(function(h){var c=map[ModelCFoundation_normHeader_(h)];if(c===undefined)return;textDateColumns.push(c);for(var r=1;r<values.length;r++){var v=values[r][c];if(Object.prototype.toString.call(v)==='[object Date]'&&!isNaN(v.getTime()))values[r][c]=Utilities.formatDate(v,tz,'yyyy-MM-dd');else if(v!==''&&v!==null&&v!==undefined)values[r][c]=ModelCFoundation_clean_(v);}});}
  return{range:range,values:values,numberFormats:numberFormats,sheet:sheet,textDateColumns:textDateColumns};
}
function ModelCScopeOwner_restoreSnapshot_(snap){if(!snap||!snap.sheet||!snap.range)return;snap.sheet.clearContents();if(snap.numberFormats)snap.range.setNumberFormats(snap.numberFormats);(snap.textDateColumns||[]).forEach(function(c){if(snap.values.length>1)snap.sheet.getRange(2,c+1,snap.values.length-1,1).setNumberFormat('@');});snap.range.setValues(snap.values);}
