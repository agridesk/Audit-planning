 ...
 * BUILD: 2026-09-17_AUDITOR_PORTAL_SCROLL_STABILITY_TESTS_R2_CONTRACT_DRIFT_GUARD
 ***********************************************************************/
var AUDITOR_SCROLL_TEST_BUILD='2026-09-17_AUDITOR_PORTAL_SCROLL_STABILITY_TESTS_R2_CONTRACT_DRIFT_GUARD';
function RUN_AUDITOR_PORTAL_SCROLL_STABILITY_REGRESSION(){
  ...
  var render=String(src.match(/function\s+renderGrid\s*\(\s*rows\s*\)\s*\{[\s\S]*?\/\/\s*Client-side grid cache/)||'');
  ...
  t('renderGridFound',/function\s+renderGrid\s*\(\s*rows\s*\)/.test(render));
