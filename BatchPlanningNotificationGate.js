 ...
 * BUILD: 2026-09-18_BATCH_PLANNING_NOTIFICATION_GATE_R2_ITEM_ISOLATION
 * Defers manager PLAN notifications during a private Batch transaction.
 */
var BATCH_PLANNING_NOTIFICATION_GATE_BUILD='2026-09-18_BATCH_PLANNING_NOTIFICATION_GATE_R2_ITEM_ISOLATION';
...
function BatchPlanningNotificationGate_flush_(items){var out=[];(items||[]).forEach(function(x){try{out.push(StatusNotificationBridge_Dispatch_(x.action,x.actor,x.ctx,x.payload,x.result));}catch(e){out.push({success:false,error:'POST_COMMIT_NOTIFICATION_ITEM_FAILED',message:String(e&&e.message||e)});}});return out;}
