 ...
 * BUILD: 2026-09-17_BATCH_PLANNING_ROUTE_MATRIX_R1
 *
 * Canonical Batch Planning route adapter.
 * - Google Routes API / Compute Route Matrix
 * - GOOGLE_MAPS_API_KEY from Script Properties (existing Company Map key)
 * - travel time is leading; distance informational
 * - cache required; no planning writes
 */
var BATCH_PLANNING_ROUTE_MATRIX_BUILD='2026-09-17_BATCH_PLANNING_ROUTE_MATRIX_R1';
...
function BatchPlanningRouteMatrix_Get(origin,destination,options){
 ...
 var apiKey=String(PropertiesService.getScriptProperties().getProperty('GOOGLE_MAPS_API_KEY')||'').trim();
 if(!apiKey)return{ok:false,error:'GOOGLE_MAPS_API_KEY_MISSING',cacheHit:false};
 ...
 if(code<200||code>=300)return{ok:false,error:'ROUTES_API_HTTP_'+code,httpCode:code,detail:String(text||'').slice(0,500),cacheHit:false};
...
function BatchPlanningRouteMatrix_GetConfig(){var key=String(PropertiesService.getScriptProperties().getProperty('GOOGLE_MAPS_API_KEY')||'').trim();return{ok:true,build:BATCH_PLANNING_ROUTE_MATRIX_BUILD,provider:'Google Routes API / Compute Route Matrix',apiKeyConfigured:!!key,cacheRequired:true,travelTimeLeading:true,distanceInformational:true};}
