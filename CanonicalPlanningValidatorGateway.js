 ...
 * BUILD: 2026-09-16_ROADMAP_2_4_CANONICAL_VALIDATOR_GATEWAY_R4_ROTATION_RELEASE_GATE
 *
 * PURPOSE
 *   Audit-level read-only gateway for shared planning validators.
 *
 * GOVERNANCE
 *   Rotation remains ADVISORY by default. Tiered rotation is available only
 *   through an explicit rotationPolicyMode='TIERED' opt-in. This prevents the
 *   presence of RotationGovernanceService from silently changing current
 *   planning/commit semantics before the governed release.
 *
 * NO writes. NO lifecycle changes. NO Availability writes.
 ***********************************************************************/

var CANONICAL_VALIDATOR_GATEWAY_BUILD = '2026-09-16_ROADMAP_2_4_CANONICAL_VALIDATOR_GATEWAY_R4_ROTATION_RELEASE_GATE';
...
function CPVG_readAudit_(auditId, perf) {
  ...
  if (typeof __mp_getAuditPlanningRow_ === 'function') {
    ...
    if (targeted && targeted.row) {
      ...
      return { ss:ss, headers:targeted.hdr || [], row:targeted.row, rowNumber:targeted.rowNumber || 0, source:'__mp_getAuditPlanningRow_' };
    }
  }
  ...
  for (var r = 1; r < values.length; r++) {
    if (CPVG_clean_(values[r][cAuditId]) === auditId) {
      ...
      return { ss:ss, headers:headers, row:values[r], rowNumber:r + 1, source:'Audit planning fallback scan' };
    }
  }
...
function CPVG_requiredScopes_(headers, row) {
  ...
  return scopes.map(function(s) { return s ? CPVG_clean_(s.name || s.code || s.slot || '') : ''; }).filter(function(x) { return !!x; });
}
...
function CPVG_rotationMetadataUnavailable_(ctx) {
  return CanonicalValidator_warning(
    ...
    { subject:CPV_subject_(ctx), source:'rotation governance / existing eligibility metadata', evidence:null }
  );
}

function CPVG_rotationPolicyMode_(ctx) {
  var mode = CPVG_clean_(ctx && ctx.rotationPolicyMode).toUpperCase();
  return mode === 'TIERED' ? 'TIERED' : 'ADVISORY';
}
...
function CPVG_rotationVerdict_(ctx, qualificationVerdict) {
  ...
  /* Tiered policy is release-gated. Merely loading RotationGovernanceService
     must never change current planning semantics. */
  if (CPVG_rotationPolicyMode_(ctx) === 'TIERED') {
    if (typeof RotationGovernanceService_worstVerdict !== 'function') {
      return CanonicalValidator_hardBlock(
        CanonicalValidatorKind.ROTATION,
        'ROTATION_GOVERNANCE_UNAVAILABLE',
        'Tiered rotation was requested but RotationGovernanceService is unavailable',
        { subject:CPV_subject_(ctx), source:'RotationGovernanceService', evidence:null }
      );
    }
    try {
  ...
  var rotationMeta = explicitRotation != null ? explicitRotation : CPVG_rotationFromQualification_(qualificationVerdict);
  if (rotationMeta) {
    return CPV_rotation({
      auditId:ctx.auditId,
      auditorEmail:ctx.auditorEmail,
...
function CanonicalPlanningValidators_evaluateAudit(input) {
  ...
  var ctx = {
    ...
    rotationPolicyMode:CPVG_clean_(input.rotationPolicyMode).toUpperCase() || 'ADVISORY'
  };
  if (typeof DPL_mark_ === 'function') DPL_mark_(__perf, 'contextPrepared', { scopeCount:ctx.requiredScopes.length, blockCount:ctx.blocks.length, rotationPolicyMode:CPVG_rotationPolicyMode_(ctx) });
  var verdicts = [];
  ...
  if (include.rotation !== false) {
    ...
    if (typeof DPL_mark_ === 'function') DPL_mark_(__perf, 'rotation', { level:rotationVerdict.level, mode:CPVG_rotationPolicyMode_(ctx) });
  }
  ...
  aggregate.rotationPolicyMode = CPVG_rotationPolicyMode_(ctx);
  ...
  if (typeof DPL_end_ === 'function') {
    var perfPayload = DPL_end_(__perf, { overallLevel:aggregate.overallLevel, verdictCount:verdicts.length, contextSource:audit.source, rotationPolicyMode:aggregate.rotationPolicyMode });
