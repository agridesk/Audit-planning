/***********************************************************************
 * FILE: CompaniesIndexService.gs | BUILD: COMPANIES_INDEX_V4_AMS01_20260907
 *
 * PURPOSE
 ...
 *
 * AMS-01 2026-09-07:
 * - NameCore cold build now uses ONE Companies sheet read.
 * - Previous route built UidCore + LocationSummary separately, causing two
 *   full Companies reads and oversized intermediate cache payloads.
 * - Output contract remains identical: merged core + location summary by name.
 ***********************************************************************/
...
function CompaniesIndex_GetNameCoreIndex(forceRefresh) {
  ...
  var out = CompaniesIndex_buildNameCoreIndexSingleRead_();
  COMPANIESINDEX_EXEC_CACHE.nameCore = out;
  CompaniesIndex_cachePut_(key, out);
  return out;
}

function CompaniesIndex_buildNameCoreIndexSingleRead_() {
  ...
  var out = {
    ...
    byName: {},
    error: pack.error || '',
    build: 'COMPANIES_INDEX_V4_AMS01_SINGLE_READ'
  };
  ...
  for (var i = 0; i < rows.length; i++) {
    ...
    var summary = CompaniesIndex_parseLocationSummary_(rawJson, core.location, core.gpsData);
    var loc = {
      companyUid: uid,
      companyName: companyName,
    ...
    out.byName[CompaniesIndex_nameKey_(companyName)] = CompaniesIndex_mergeCoreAndLocation_(core, loc);
    out.count++;
  }
...
function CompaniesIndex_buildUidNameIndex_() {
  var pack = CompaniesIndex_readCompanies_();
  var out = { ok:pack.ok, sheetName:COMPANIESINDEX_SHEET_NAME, generatedAt:new Date().toISOString(), count:0, byUid:{}, list:[], error:pack.error || '' };
  if (!pack.ok) return out;
  var idx = pack.idx, rows = pack.rows;
  for (var i=0;i<rows.length;i++) {
    var row = rows[i] || [];
    var uid = CompaniesIndex_clean_(CompaniesIndex_val_(row, idx, ['Company_UID','Company UID','CompanyUID','COMPANY_UID','UID']));
    var companyName = CompaniesIndex_clean_(CompaniesIndex_val_(row, idx, ['Company','Company name','Name','Bedrijf','COMPANY']));
    if (!uid || !companyName) continue;
    var rec = { companyUid:uid, companyName:companyName };
    out.byUid[uid] = rec; out.list.push(rec); out.count++;
  }
  out.list.sort(function(a,b){ return String(a.companyName || '').localeCompare(String(b.companyName || '')); });
  return out;
}

function CompaniesIndex_buildUidCoreIndex_() {
  var pack = CompaniesIndex_readCompanies_();
  var out = { ok:pack.ok, sheetName:COMPANIESINDEX_SHEET_NAME, generatedAt:new Date().toISOString(), count:0, byUid:{}, error:pack.error || '' };
  if (!pack.ok) return out;
  var idx = pack.idx, rows = pack.rows;
  for (var i=0;i<rows.length;i++) {
    var row = rows[i] || [];
...
function CompaniesIndex_buildLocationSummaryIndex_() {
  ...
  for (var i=0;i<rows.length;i++) {
    ...
    var companyName = CompaniesIndex_clean_(CompaniesIndex_val_(row, idx, ['Company','Company name','Name','Bedrijf','COMPANY']));
    if (!uid || !companyName) continue;
    var rawJson = CompaniesIndex_clean_(CompaniesIndex_val_(row, idx, ['Locations_JSON','Locations JSON','LocationsJSON']));
    var legacyLocation = CompaniesIndex_clean_(CompaniesIndex_val_(row, idx, ['Location','HQ Location','HQ']));
    var legacyGps = CompaniesIndex_clean_(CompaniesIndex_val_(row, idx, ['GPS-data','GPS data','GPS','GPS HQ']));
    var summary = CompaniesIndex_parseLocationSummary_(rawJson, legacyLocation, legacyGps);
    out.byUid[uid] = { companyUid:uid, companyName:companyName, locationsCount:summary.locationsCount, activeLocationsCount:summary.activeLocationsCount, hqLabel:summary.hqLabel, hqGps:summary.hqGps, hasLocationsJson:summary.hasLocationsJson, locations:summary.locations };
    out.count++;
  }
  return out;
}
...
function CompaniesIndex_parseLocationSummary_(rawJson, legacyLocation, legacyGps) {
  var out = { hasLocationsJson:false, locationsCount:0, activeLocationsCount:0, hqLabel:legacyLocation || '', hqGps:legacyGps || '', locations:[] };
  if (!rawJson) {
    if (legacyLocation || legacyGps) {
      out.locations = [{ code:'HQ', label:legacyLocation || '', gps:legacyGps || '', comment:'' }];
      out.locationsCount = 1; out.activeLocationsCount = 1;
    }
    return out;
  }
  try {
    var parsed = JSON.parse(rawJson), list = [];
    if (parsed && Array.isArray(parsed.locations)) list = parsed.locations;
    ...
    for (var i=0;i<list.length;i++) {
      ...
      if (!(label || gps || comment)) continue;
      out.locations.push({ code:code || ('L' + (i+1)), label:label, gps:gps, comment:comment });
    }
    out.locationsCount = out.locations.length;
    out.activeLocationsCount = out.locations.length;
    var hq = null;
    for (var j=0;j<out.locations.length;j++) if (String(out.locations[j].code || '').toUpperCase() === 'HQ') { hq = out.locations[j]; break; }
    if (!hq && out.locations.length) hq = out.locations[0];
    if (hq) { out.hqLabel = hq.label || legacyLocation || ''; out.hqGps = hq.gps || legacyGps || ''; }
    return out;
  } catch (e) {
    if (legacyLocation || legacyGps) {
      out.locations = [{ code:'HQ', label:legacyLocation || '', gps:legacyGps || '', comment:'' }];
...
function CompaniesIndex_readCompanies_() {
  ...
  if (!sh) return { ok:false, error:'Missing sheet: ' + COMPANIESINDEX_SHEET_NAME, headers:[], idx:{}, rows:[] };
  var values = sh.getDataRange().getValues();
  if (!values || !values.length) return { ok:true, headers:[], idx:{}, rows:[] };
  var headers = values[0] || [];
  return { ok:true, headers:headers, idx:CompaniesIndex_headerMap_(headers), rows:values.length > 1 ? values.slice(1) : [] };
}

function CompaniesIndex_headerMap_(headers) {
  var map = {}; headers = headers || [];
  for (var i=0;i<headers.length;i++) {
    var raw = String(headers[i] || '').trim();
    if (!raw) continue;
    map[raw] = i; map[raw.toUpperCase()] = i; map[raw.toLowerCase()] = i; map[CompaniesIndex_normKey_(raw)] = i;
  }
  return map;
}
...
function CompaniesIndex_normKey_(v) {
  return String(v == null ? '' : v).replace(/\u00A0/g,' ').replace(/[\u200B-\u200D\uFEFF]/g,'').trim().toLowerCase().replace(/[^a-z0-9]+/g,'');
}
function CompaniesIndex_nameKey_(v) { return CompaniesIndex_clean_(v).toLowerCase(); }
function CompaniesIndex_clean_(v) { return String(v == null ? '' : v).trim(); }
function CompaniesIndex_cacheGet_(keyPart) {
  try { if (typeof AUDIT_CACHE !== 'undefined' && AUDIT_CACHE && typeof AUDIT_CACHE.get === 'function') return AUDIT_CACHE.get(COMPANIESINDEX_CACHE_NAMESPACE, keyPart); } catch (e) {}
  return null;
}
function CompaniesIndex_cachePut_(keyPart, value) {
  try { if (typeof AUDIT_CACHE !== 'undefined' && AUDIT_CACHE && typeof AUDIT_CACHE.put === 'function') AUDIT_CACHE.put(COMPANIESINDEX_CACHE_NAMESPACE, keyPart, value, COMPANIESINDEX_CACHE_TTL_SECONDS); } catch (e) {}
}
...
function RUN_COMPANIESINDEX_DIAGNOSTICS() {
  ...
  var out = { ok:!!uidName.ok && !!uidCore.ok && !!locations.ok && !!nameCore.ok, durationMs:new Date().getTime()-started, auditCacheAvailable:{ object:(typeof AUDIT_CACHE !== 'undefined' && !!AUDIT_CACHE), get:(typeof AUDIT_CACHE !== 'undefined' && AUDIT_CACHE && typeof AUDIT_CACHE.get === 'function'), put:(typeof AUDIT_CACHE !== 'undefined' && AUDIT_CACHE && typeof AUDIT_CACHE.put === 'function'), removeNamespace:(typeof AUDIT_CACHE !== 'undefined' && AUDIT_CACHE && typeof AUDIT_CACHE.removeNamespace === 'function') }, uidNameCount:uidName.count || 0, uidCoreCount:uidCore.count || 0, locationSummaryCount:locations.count || 0, nameCoreCount:nameCore.count || 0, sampleCompanies:(uidName.list || []).slice(0,10), sampleLocationSummaries:Object.keys(locations.byUid || {}).slice(0,5).map(function(uid){ return locations.byUid[uid]; }) };
...
function RUN_COMPANIESINDEX_CACHE_WARMUP() {
  ...
  var out = { ok:!!uidName.ok && !!uidCore.ok && !!locations.ok && !!nameCore.ok, durationMs:new Date().getTime()-started, uidNameCount:uidName.count || 0, uidCoreCount:uidCore.count || 0, locationSummaryCount:locations.count || 0, nameCoreCount:nameCore.count || 0, auditCacheAvailable:{ object:(typeof AUDIT_CACHE !== 'undefined' && !!AUDIT_CACHE), get:(typeof AUDIT_CACHE !== 'undefined' && AUDIT_CACHE && typeof AUDIT_CACHE.get === 'function'), put:(typeof AUDIT_CACHE !== 'undefined' && AUDIT_CACHE && typeof AUDIT_CACHE.put === 'function'), removeNamespace:(typeof AUDIT_CACHE !== 'undefined' && AUDIT_CACHE && typeof AUDIT_CACHE.removeNamespace === 'function') } };
  Logger.log(JSON.stringify(out, null, 2)); return out;
}
function RUN_COMPANIESINDEX_CACHE_HIT_TEST() {
  COMPANIESINDEX_EXEC_CACHE.uidName = null; COMPANIESINDEX_EXEC_CACHE.uidCore = null; COMPANIESINDEX_EXEC_CACHE.locationSummary = null; COMPANIESINDEX_EXEC_CACHE.nameCore = null;
  var started = new Date().getTime();
  var uidName = CompaniesIndex_GetUidNameIndex(false);
  var uidCore = CompaniesIndex_GetUidCoreIndex(false);
  var locations = CompaniesIndex_GetLocationSummaryIndex(false);
...
function RUN_COMPANIESINDEX_CLEARCACHE() { return CompaniesIndex_ClearCache(); }
