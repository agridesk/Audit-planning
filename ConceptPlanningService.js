 ...
 *   - No synchronous eligibility recomputation in this hot path.
...
function CPS_rankAuditors_(auditors) {
  ...
  out.sort(function(a, b) {
    ...
    return CPS_clean_(a && (a.name || a.email)).localeCompare(CPS_clean_(b && (b.name || b.email)));
  });
  return out;
}
...
function CPS_advisoryCandidate_(a) {
  return {
    name: CPS_clean_(a && a.name), email: CPS_norm_(a && a.email),
...
function CPS_demandAuditIds_(rows) {
  ...
  for (var i=0;i<(rows||[]).length;i++) {
    ...
    if (!id||seen[id]) continue; seen[id]=true; out.push(id);
  }
  return out;
}
...
function CPS_refreshPolicy_(eligibility) {
  var ids=[]; var rows=eligibility&&eligibility.rows?eligibility.rows:[];
  for(var i=0;i<rows.length;i++) if(rows[i]&&rows[i].requiresCanonicalRefresh===true) ids.push(CPS_clean_(rows[i].auditId));
  var missing=eligibility&&eligibility.missingAuditIds?eligibility.missingAuditIds:[];
  for(var m=0;m<missing.length;m++) ids.push(CPS_clean_(missing[m]));
  var seen={}, clean=[];
  for(var j=0;j<ids.length;j++){if(ids[j]&&!seen[ids[j]]){seen[ids[j]]=true;clean.push(ids[j]);}}
  return {
    ...
    recommendedMode:'bounded recovery/warm-up outside primary planning request'
  };
}

function ConceptPlanningService_get(input) {
  input=input||{};
  var perf=(typeof DPL_start_==='function')?DPL_start_('ConceptPlanningService_get',{from:input.from||input.start||input.periodFrom||'',to:input.to||input.end||input.periodTo||''}):null;
  if(typeof PlanningDemandService_get!=='function')throw new Error('ConceptPlanningService: PlanningDemandService_get unavailable');
  if(typeof EligibilityBatchReadModel_get!=='function')throw new Error('ConceptPlanningService: EligibilityBatchReadModel_get unavailable');
  ...
  if(typeof DPL_mark_==='function')DPL_mark_(perf,'eligibilityBatch',{requested:auditIds.length,returned:eligibility&&eligibility.rows?eligibility.rows.length:0,stale:eligibility&&eligibility.meta?eligibility.meta.stale||0:0,missing:eligibility&&eligibility.meta?eligibility.meta.missing||0:0,parseErrors:eligibility&&eligibility.meta?eligibility.meta.parseErrors||0:0,refreshRequired:eligibility&&eligibility.meta?eligibility.meta.refreshRequired||0:0});

  var rows=[],ready=0,refreshRequired=0,noCandidates=0;
  var maxCandidates=Math.max(1,Math.min(20,Number(input.maxCandidates||5)||5));
  for(var i=0;i<demandRows.length;i++){
    var d=demandRows[i]||{},id=CPS_clean_(d.auditId),e=eligibility&&eligibility.byAuditId?eligibility.byAuditId[id]:null;
    var reason='',candidates=[],requiresRefresh=false,refreshReasons=[];
    if(!e){requiresRefresh=true;reason='ELIGIBILITY_CACHE_MISSING';refreshReasons=['CACHE_MISSING'];}
    else if(e.requiresCanonicalRefresh===true||e.stale===true){requiresRefresh=true;reason='ELIGIBILITY_REFRESH_REQUIRED';refreshReasons=Array.isArray(e.refreshReasons)?e.refreshReasons.slice():['REFRESH_REQUIRED'];}
    else{var ranked=CPS_rankAuditors_(e.auditors||[]);candidates=ranked.slice(0,maxCandidates).map(CPS_advisoryCandidate_);if(!candidates.length)reason='NO_ELIGIBLE_AUDITORS';}
    if(requiresRefresh)refreshRequired++;else if(!candidates.length)noCandidates++;else ready++;
    rows.push({auditId:id,companyUid:CPS_clean_(d.companyUid),company:CPS_clean_(d.company),country:CPS_clean_(d.country),region:CPS_clean_(d.region),status:CPS_clean_(d.status),planningWindowFrom:CPS_clean_(d.planningWindowFrom),planningWindowTo:CPS_clean_(d.planningWindowTo),urgency:CPS_clean_(d.urgency),scopes:Array.isArray(d.scopes)?d.scopes.slice():[],hoursToPlan:Number(d.hoursToPlan||0)||0,candidateAuditors:candidates,advisoryState:requiresRefresh?'REFRESH_REQUIRED':(candidates.length?'READY':'NO_CANDIDATES'),advisoryReason:reason,eligibilityStale:!!(e&&e.stale===true),eligibilityComputedAt:CPS_clean_(e&&e.computedAt),requiresCanonicalRefresh:requiresRefresh,eligibilityRefreshReasons:refreshReasons});
  }
