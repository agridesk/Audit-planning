 ...
 * BUILD: 2026-09-14_ROADMAP_2_4_CANONICAL_MODIFY_R7_WINDOW_LEADING
 *
 ...
 ***********************************************************************/
var PLANNING_CANONICAL_MODIFY_BUILD='2026-09-14_ROADMAP_2_4_CANONICAL_MODIFY_R7_WINDOW_LEADING';
...
function PlanningCanonicalModifyService_modify(input){
  ...
  return Platform_withLock('planning-modify:'+auditId,function(){
    ...
    if(!gate||gate.canCommit!==true)return{success:true,build:PLANNING_CANONICAL_MODIFY_BUILD,auditId:auditId,modified:false,reason:gate&&gate.reason||'MODIFY_PREFLIGHT_BLOCKED',gate:gate||null,meta:{writes:false,lockUsed:true,planningWindowLeading:true}};
    ...
    if(oldOwner&&newOwner&&oldOwner!==newOwner)return{success:true,build:PLANNING_CANONICAL_MODIFY_BUILD,auditId:auditId,modified:false,reason:'AUDITOR_CHANGE_NOT_SUPPORTED_R7',canonicalStatus:rowInfo.status,meta:{writes:false,lockUsed:true,sameAuditorOnly:true}};
    ...
    if(!validation||validation.success!==true){
      return PCMOD_failure_(auditId,validation&&validation.code||'CANONICAL_ROW_VALIDATION_FAILED',validation&&validation.message||'Canonical row validation failed.',{
        ...
        gate:gate,
        meta:{writes:false,lockUsed:true,validatedBeforeAvailability:true,availabilityMutated:false,planningWindowLeading:true}
      });
    }
