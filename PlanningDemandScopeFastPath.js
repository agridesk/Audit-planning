 ...
 * BUILD: 2026-09-17_AMS01_PLANNING_DEMAND_SCOPE_FASTPATH_R2_CANONICAL_EVIDENCE
 *
 * PURPOSE
 * - Execution-local scope projection for PlanningDemandService.
 * - Build the Audit planning scope-column plan once from canonical
 *   Config_Scopes evidence already available in the execution cache.
 * - Preserve Config_Scopes as the only scope metadata owner.
 * - No persistent cache, no writes, no new SSoT.
 ***********************************************************************/
var AMS01_PDS_SCOPE_FASTPATH_BUILD='2026-09-17_AMS01_PLANNING_DEMAND_SCOPE_FASTPATH_R2_CANONICAL_EVIDENCE';
...
function AMS01_PDS_SF_catalog_(){if(typeof CONFIGSCOPES_EXEC_CACHE!=='undefined'&&CONFIGSCOPES_EXEC_CACHE&&CONFIGSCOPES_EXEC_CACHE.catalog)return CONFIGSCOPES_EXEC_CACHE.catalog;if(typeof ConfigScopes_loadCatalogFromSheet_==='function'){var c=ConfigScopes_loadCatalogFromSheet_()||{rows:[]};if(typeof CONFIGSCOPES_EXEC_CACHE!=='undefined'&&CONFIGSCOPES_EXEC_CACHE)CONFIGSCOPES_EXEC_CACHE.catalog=c;return c;}if(typeof ConfigScopes_GetCatalog==='function')return ConfigScopes_GetCatalog(false)||{rows:[]};return{rows:[]};}
function AMS01_PDS_scopePlan_(headers){headers=headers||[];var key=headers.map(function(x){return AMS01_PDS_SF_clean_(x);}).join('\u001f');if(AMS01_PDS_SCOPE_FASTPATH_CACHE[key])return AMS01_PDS_SCOPE_FASTPATH_CACHE[key];var catalog=AMS01_PDS_SF_catalog_(),rows=catalog&&Array.isArray(catalog.rows)?catalog.rows:[],byKey={};for(var i=0;i<rows.length;i++){var r=rows[i]||{};if(typeof ConfigScopes_yes_==='function'&&(!ConfigScopes_yes_(r.active,true)||ConfigScopes_yes_(r.archived,false)))continue;var name=AMS01_PDS_SF_clean_(r.displayName||r.name||r.scopeCode||r.slotKey);if(!name)continue;[r.displayName,r.name,r.scopeCode,r.slotKey].forEach(function(v){var n=AMS01_PDS_SF_norm_(v);if(n)byKey[n]=name;});}var columns=[];for(var h=0;h<headers.length;h++){var n=AMS01_PDS_SF_norm_(headers[h]),name=byKey[n];if(name)columns.push({col:h,name:name});}var plan={source:'Config_Scopes canonical execution evidence',configuredScopes:rows.length,matchedColumns:columns.length,columns:columns,build:AMS01_PDS_SCOPE_FASTPATH_BUILD};AMS01_PDS_SCOPE_FASTPATH_CACHE[key]=plan;return plan;}
