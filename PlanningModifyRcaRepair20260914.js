 ...
 * BUILD: 2026-09-14_MODIFY_RCA_REPAIR_R2_RELEASE_RECONCILE
 *
 * DEV verification after Modify RCA:
 ...
 * - verify legacy orphan Availability rows 14/15/19 Sep are no longer hard;
 * - verify previously repaired 16/17/18 Sep remain clear;
 * - run grouped Modify regressions plus release-reconcile regressions.
 ***********************************************************************/
var PLANNING_MODIFY_RCA_REPAIR_BUILD='2026-09-14_MODIFY_RCA_REPAIR_R2_RELEASE_RECONCILE';
...
function RUN_PLANNING_MODIFY_RCA_REPAIR_AND_VERIFY(){
  ...
  var out={ok:false,build:PLANNING_MODIFY_RCA_REPAIR_BUILD,cacheGeneration:'',recalculate:null,monterosa:null,availability:null,regression:null,releaseRegression:null,errors:[],meta:{writes:true,persistenceWrites:'Planning window derived columns only',cacheInvalidated:true,newSsot:false}};
  try{
    ...
    if(typeof RUN_PLANNING_AVAILABILITY_RELEASE_RECONCILE_REGRESSION!=='function')throw new Error('RUN_PLANNING_AVAILABILITY_RELEASE_RECONCILE_REGRESSION unavailable');
    ...
    var raw=AvailabilityService.getAuditorAvailabilityRaw(auditor,'2026-09-14','2026-09-19',{}),days=raw&&raw.days||{},hard=[];
    ['2026-09-14','2026-09-15','2026-09-16','2026-09-17','2026-09-18','2026-09-19'].forEach(function(d){
    ...
    out.availability={success:true,hardConflicts:hard,daysChecked:['2026-09-14','2026-09-15','2026-09-16','2026-09-17','2026-09-18','2026-09-19']};
    out.regression=RUN_PLANNING_MODIFY_WINDOW_VERIFICATION();
    out.releaseRegression=RUN_PLANNING_AVAILABILITY_RELEASE_RECONCILE_REGRESSION();
    ...
    var releaseRegressionOk=out.releaseRegression&&out.releaseRegression.ok===true;
    out.ok=!!(windowOk&&availabilityOk&&recalcOk&&regressionOk&&releaseRegressionOk);
    ...
    if(!availabilityOk)out.errors.push('Hard Availability rows remain on released Monterosa dates 14-19 Sep');
    if(!recalcOk)out.errors.push('Planning-window rebuild failed');
    if(!regressionOk)out.errors.push('Modify regression failed');
    if(!releaseRegressionOk)out.errors.push('Availability release-reconcile regression failed');
