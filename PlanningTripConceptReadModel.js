...
function PlanningTripConceptReadModel_get(input){
 ...
 (profile.companies||[]).forEach(function(x){var uid=PTC_s_(x.companyUid||x.uid),gps=PTC_s_(x.gps||x.coordinates||x.GPS),m=gps.match(/(-?\d+(?:\.\d+)?)\s*[,; ]\s*(-?\d+(?:\.\d+)?)/);if(uid&&m){loc[uid]=[{companyId:uid,locationCode:'HQ',locationLabel:'HQ',lat:Number(m[1]),lng:Number(m[2]),gpsValid:true,mapsUrl:'https://www.google.com/maps?q='+m[1]+','+m[2]}];}});
 rows.forEach(function(r){var places=loc[PTC_s_(r.companyUid)]||[],place=places[0]||null;r.location=place?{code:'HQ',label:'HQ',lat:place.lat,lng:place.lng,gpsValid:true,mapsUrl:place.mapsUrl}:{code:'',label:'',lat:null,lng:null,gpsValid:false,mapsUrl:''};});
