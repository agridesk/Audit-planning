 ...
 ***********************************************************************/
var AMS03_TRIP_CONCEPT_BUILD='2026-09-23_AMS03_PHASE8_TRIP_CONCEPT_R3_BOUNDED_LOCATION_JSON';
...
function PTC_locationsFromProfile_(x){
 var out=[],raw=PTC_s_(x&&x.locationsJson);if(raw){try{var a=JSON.parse(raw);if(!Array.isArray(a)&&a&&Array.isArray(a.locations))a=a.locations;if(Array.isArray(a))a.forEach(function(z){z=z||{};var lat=PTC_num_(z.lat!=null?z.lat:z.latitude),lng=PTC_num_(z.lng!=null?z.lng:z.longitude);if(lat!=null&&lng!=null)out.push({locationCode:PTC_s_(z.code||z.locationCode||'HQ'),locationLabel:PTC_s_(z.name||z.label||z.location||z.code||'HQ'),lat:lat,lng:lng,gpsValid:true,mapsUrl:'https://www.google.com/maps?q='+lat+','+lng});});}catch(e){}}
 if(!out.length){var h=PTC_s_(x&&x.hqLocation),m=h.match(/(-?\d+(?:\.\d+)?)\s*[,; ]\s*(-?\d+(?:\.\d+)?)/);if(m)out.push({locationCode:'HQ',locationLabel:'HQ',lat:Number(m[1]),lng:Number(m[2]),gpsValid:true,mapsUrl:'https://www.google.com/maps?q='+m[1]+','+m[2]});}
 return out;
}
