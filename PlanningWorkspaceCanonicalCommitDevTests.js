 ...
 * BUILD: 2026-09-09_PLANNING_WORKSPACE_2_0_CANONICAL_COMMIT_DEV_TEST_R4_TARGETED_REFRESH
 * CONTROLLED DESTRUCTIVE DEV TEST. Business truth write occurs only after
 * explicit target, Pending Planning, targeted eligibility refresh and preflight.
 ***********************************************************************/
var PLANNING_WORKSPACE_CANONICAL_COMMIT_DEV_TEST_BUILD='2026-09-09_PLANNING_WORKSPACE_2_0_CANONICAL_COMMIT_DEV_TEST_R4_TARGETED_REFRESH';
...
function PWCCT_firstWeekday_(f,t){var d=f;for(var i=0;i<31&&d<=t;i++,d=PWCCT_addDays_(d,1)){var w=PWCCT_weekday_(d);if(w!==0&&w!==6)return d;}return f;}
...
function RUN_PLANNING_WORKSPACE_CANONICAL_COMMIT_DEV_TEST(){
 ...
 var refresh=EligibilityTargetedRefreshService_refresh({auditIds:[auditId],maxRefresh:1,force:true,dryRun:false});
 PWCCT_assert_(r,'eligibilityRefreshSuccess',refresh&&refresh.success===true,refresh&&refresh.items&&refresh.items[0]&&refresh.items[0].error||'');PWCCT_assert_(r,'eligibilityRefreshPerformed',refresh&&refresh.refreshed===1,refresh&&refresh.items&&refresh.items[0]&&refresh.items[0].action||'');PWCCT_assert_(r,'eligibilityBusinessTruthUntouched',refresh&&refresh.meta&&refresh.meta.writesBusinessTruth===false);
 if(!(refresh&&refresh.success===true&&refresh.refreshed===1))throw new Error('Targeted eligibility refresh failed for '+auditId);
 ...
 if(!(pick&&pick.advisoryState==='READY'&&pick.candidateAuditors&&pick.candidateAuditors.length))throw new Error('Explicit DEV audit still not READY after targeted refresh: '+auditId);
 var c=pick.candidateAuditors[0],date=PWCCT_firstWeekday_(PWCCT_date_(pick.planningWindowFrom)||from,PWCCT_date_(pick.planningWindowTo)||to),hours=Math.min(8,Math.max(1,Number(pick.hoursToPlan||8)||8)),endHour=9+hours;if(endHour>17){hours=8;endHour=17;}var start='09:00',end=String(Math.floor(endHour)).padStart(2,'0')+':'+(endHour%1?'30':'00'),blocks=[{date:date,start:start,end:end,hours:hours}];
 ...
 var failed=r.filter(function(x){return !x.ok;}),out={ok:failed.length===0,build:PLANNING_WORKSPACE_CANONICAL_COMMIT_DEV_TEST_BUILD,total:r.length,passed:r.length-failed.length,failed:failed.length,results:r,meta:{destructive:true,devOnly:true,explicitAudit:true,targetedEligibilityRefresh:true,canonicalWritePerformed:!!(commit&&commit.data&&commit.data.committed),auditId:auditId,auditorEmail:c.email,date:date,start:start,end:end,durationMs:Date.now()-started}};Logger.log(JSON.stringify(out,null,2));return out;
}
