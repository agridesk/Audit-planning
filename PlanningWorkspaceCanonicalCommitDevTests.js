 ...
 * BUILD: 2026-09-09_PLANNING_WORKSPACE_2_0_CANONICAL_COMMIT_DEV_TEST_R1
 *
 * CONTROLLED DESTRUCTIVE DEV TEST.
 * Selects a READY planning-demand audit, validates it, then performs exactly
 * one canonical PLAN commit through the browser RPC boundary.
 ***********************************************************************/
var PLANNING_WORKSPACE_CANONICAL_COMMIT_DEV_TEST_BUILD='2026-09-09_PLANNING_WORKSPACE_2_0_CANONICAL_COMMIT_DEV_TEST_R1';
...
function RUN_PLANNING_WORKSPACE_CANONICAL_COMMIT_DEV_TEST(){
  ...
  for(var i=0;i<rows.length;i++){if(rows[i]&&rows[i].advisoryState==='READY'&&rows[i].candidateAuditors&&rows[i].candidateAuditors.length){pick=rows[i];break;}}
  PWCCT_assert_(results,'readyAuditFound',!!pick);
  if(!pick)throw new Error('No READY audit available for controlled DEV commit');
  ...
  var pre=PlanningWorkspaceRpc_commitPreflight({auditId:pick.auditId,expectedRevision:revision,auditorEmail:candidate.email,auditorName:candidate.name,blocks:blocks,waiverAccepted:false});
