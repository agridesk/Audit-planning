 ...
 * BUILD: 2026-09-24_AMS03_PLANNING2_WORKSPACE_DECISION_R4_FOCUSED_PATH
 *
 * Compact, read-only decision projection over existing canonical/read-model
 * owners. No new SSoT, cache or write path.
 ***********************************************************************/
var PLANNING_WORKSPACE_DECISION_READ_MODEL_BUILD='2026-09-24_AMS03_PLANNING2_WORKSPACE_DECISION_R4_FOCUSED_PATH';
...
function PlanningWorkspaceDecisionReadModel_get(input){
 ...
 var t=Date.now(),a=ConceptPlanningService_get(input),rows=(a&&a.rows||[]).map(PWDRM_audit_),focusId=PWDRM_clean_(input.auditId),role=PWDRM_clean_(input.actorRole||input.role).toUpperCase(),actorEmail=PWDRM_clean_(input.actorEmail||input.auditorEmail).toLowerCase(),emails=[],seen={};
 if(focusId)rows=rows.filter(function(x){return PWDRM_clean_(x.auditId)===focusId;});
 if(role==='AUDITOR'){rows=rows.filter(function(x){var keep=[];for(var ci=0;ci<x.candidateAuditors.length;ci++)if(PWDRM_clean_(x.candidateAuditors[ci].email).toLowerCase()===actorEmail)keep.push(x.candidateAuditors[ci]);x.candidateAuditors=keep;return keep.length>0;});}
 ...
 var sourceMeta=a&&a.meta||{},meta={};for(var mk in sourceMeta)if(Object.prototype.hasOwnProperty.call(sourceMeta,mk))meta[mk]=sourceMeta[mk];meta.writes=false;meta.newSsot=false;meta.compactDecisionProjection=true;meta.sourceOwner='ConceptPlanningService_get';meta.candidateAuditors=emails.length;meta.sourceBuild=a.build||'';meta.durationMs=Date.now()-t;meta.focusAuditId=focusId;meta.focusedLaunch=!!focusId;meta.focusedSourceBounded=!!(focusId&&sourceMeta&&sourceMeta.focusedAudit===true);meta.focusedEligibilityCardinality=focusId&&sourceMeta?sourceMeta.focusedEligibilityCardinality:null;meta.roleScoped=role==='AUDITOR';meta.actorEmail=role==='AUDITOR'?actorEmail:'';var auditContextById={};for(var ai=0;ai<rows.length;ai++){var rr=rows[ai],id=PWDRM_clean_(rr.auditId);if(id)auditContextById[id]={company:rr.company,scopes:rr.scopes.slice(),status:rr.status};}var out={success:true,build:PLANNING_WORKSPACE_DECISION_READ_MODEL_BUILD,period:a.period,rows:rows,totals:a.totals||{},candidateAuditorEmails:emails,auditContextById:auditContextById,refreshPolicy:a.refreshPolicy||null,meta:meta};
 return out;
}
