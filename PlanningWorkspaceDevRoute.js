 ...
 * BUILD: 2026-09-09_PLANNING_WORKSPACE_2_0_DEV_ROUTE_R2
 *
 * DEV-only route helper. Authentication remains owned by EntryV5.
 ***********************************************************************/
var PLANNING_WORKSPACE_DEV_ROUTE_BUILD='2026-09-09_PLANNING_WORKSPACE_2_0_DEV_ROUTE_R2';
...
function PlanningWorkspaceDevRoute_render(ctx){
  ...
  ctx=ctx||{};
  var output=PlanningWorkspaceUi_render({
    email:String(ctx.email||'').trim().toLowerCase(),
    role:String(ctx.role||'').trim(),
    env:'DEV'
  });
  return output&&typeof output.getContent==='function'?output.getContent():String(output||'');
}
function PlanningWorkspaceDevRoute_contract(){
  return {build:PLANNING_WORKSPACE_DEV_ROUTE_BUILD,devOnly:true,authenticatedEntryOwner:'EntryV5',renderer:'PlanningWorkspaceUi_render',newSsot:false};
}
