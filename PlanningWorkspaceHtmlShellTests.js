...
function RUN_PLANNING_WORKSPACE_HTML_SHELL_REGRESSION(){
  ...
  t('serverSeedCarrier',html.indexOf('id="pwServerSeed"')>=0&&html.indexOf('__seedJson')>=0);
  t('seededDates',html.indexOf('__seedFrom')>=0&&html.indexOf('__seedTo')>=0);
  t('clientInclude',html.indexOf('PlanningWorkspaceClient.js')>=0);t('noExecutableInlineScript',html.indexOf('<script')<0);t('noDirectRpcInShell',html.indexOf('google.script.run')<0);
  var failed=r.filter(function(x){return!x.ok;}).length,out={ok:failed===0,build:PLANNING_WORKSPACE_HTML_SHELL_TEST_BUILD,total:r.length,passed:r.length-failed,failed:failed,results:r,meta:{nonDestructive:true,liveReadsPerformed:false,liveWritesPerformed:false,operationalLayout:true,serverSeedCarrier:true,rawJsonShell:false}};console.log(JSON.stringify(out,null,2));return out;
}
