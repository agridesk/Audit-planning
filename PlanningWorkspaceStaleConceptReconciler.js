 ...
 * BUILD: 2026-09-13_WORKSPACE_STALE_CONCEPT_RECONCILER_R2_VERIFY
 *
 * PURPOSE
 ...
 ***********************************************************************/
var PLANNING_WORKSPACE_STALE_CONCEPT_RECONCILER_BUILD='2026-09-13_WORKSPACE_STALE_CONCEPT_RECONCILER_R2_VERIFY';
...
function PlanningWorkspaceStaleConceptReconciler_verify(){
  var pack=PWSCR_read_(),lc=pack.lifecycle||{},active=Array.isArray(lc.activeRows)?lc.activeRows:[],stale=Array.isArray(lc.staleRows)?lc.staleRows:[],eligible=[];
  for(var i=0;i<stale.length;i++){var c=PWSCR_candidate_(stale[i]);if(c.releaseEligible)eligible.push(c);}
  var out={ok:eligible.length===0,build:PLANNING_WORKSPACE_STALE_CONCEPT_RECONCILER_BUILD,sourceActiveCount:pack.source.length,lifecycleActiveCount:active.length,lifecycleStaleCount:stale.length,remainingReleaseEligibleCount:eligible.length,active:active.map(function(x){return{auditId:PWSCR_clean_(x.auditId),canonicalStatus:PWSCR_clean_(x.canonicalStatus)};}),remainingReleaseEligible:eligible,meta:{readOnly:true,conceptWrites:false,auditPlanningWrites:false,availabilityWrites:false,statusWrites:false,expectedAfterApply:'only canonical Pending Planning concepts remain ACTIVE'}};
  console.log(JSON.stringify(out,null,2));return out;
}
...
function RUN_PLANNING_WORKSPACE_STALE_CONCEPT_RECONCILER_VERIFY(){return PlanningWorkspaceStaleConceptReconciler_verify();}
