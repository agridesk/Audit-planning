 ...
 * BUILD: 2026-09-12_WORKSPACE_TRACECERT_ELIGIBILITY_DIAG_R1
 * Read-only diagnostic for Florimark Tracecert -> Leen qualification path.
 ***********************************************************************/
var WORKSPACE_TRACECERT_ELIGIBILITY_DIAG_BUILD='2026-09-12_WORKSPACE_TRACECERT_ELIGIBILITY_DIAG_R1';
...
function RUN_WORKSPACE_TRACECERT_ELIGIBILITY_DIAGNOSTIC(){
  ...
  var cache=[];if(ec&&audits.length){var ev=ec.getDataRange().getValues(),eh=ev[0]||[],eId=WTED_find_(eh,['Audit_ID','Audit ID']),eJson=WTED_find_(eh,['Eligible_Auditors_JSON']),eStale=WTED_find_(eh,['Stale']),eBuild=WTED_find_(eh,['Computed_Build']);var wanted={};audits.forEach(function(a){wanted[a.auditId]=1;});for(var er=1;er<ev.length;er++){var id=eId>=0?WTED_clean_(ev[er][eId]):'';if(!wanted[id])continue;var names=[];try{var parsed=eJson>=0?JSON.parse(WTED_clean_(ev[er][eJson])||'[]'):[];if(Array.isArray(parsed))names=parsed.map(function(x){return WTED_clean_(x&&(x.name||x.email));});}catch(e){}cache.push({auditId:id,stale:eStale>=0?WTED_clean_(ev[er][eStale]):'',computedBuild:eBuild>=0?WTED_clean_(ev[er][eBuild]):'',eligibleAuditors:names});}}
  var out={ok:true,build:WORKSPACE_TRACECERT_ELIGIBILITY_DIAG_BUILD,audits:audits,leen:leen,eligibilityCache:cache,meta:{readOnly:true,writes:false,auditCount:audits.length}};
