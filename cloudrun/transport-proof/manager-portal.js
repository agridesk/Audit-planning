...
function render(rows){document.getElementById("rows").innerHTML=rows.map(function(r){return "<tr><td>"+esc(r.company)+"</td><td>"+esc(r.region)+"</td><td>"+esc(r.scopesText)+"</td><td>"+esc(r.status)+"</td><td>"+esc(r.planningWindowText)+"</td><td>"+esc(r.requiredHours)+"</td><td>"+esc(r.hoursPlanned)+"</td><td>"+esc(r.assignedTo)+"</td></tr>"}).join("")}
