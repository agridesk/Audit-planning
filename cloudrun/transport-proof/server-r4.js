...
const BUILD='2026-09-25_AMS_CLOUD_RUN_MANAGER_PORTAL_R27_ACTION_UI_FIX';
...
http.createServer(async(req,res)=>{if(req.method==='OPTIONS'){if(!ORIGIN)return send(res,403,{ok:false,error:'CORS_DISABLED'});res.writeHead(204,{'access-control-allow-origin':ORIGIN,'access-control-allow-methods':'GET,POST,OPTIONS','access-control-allow-headers':'content-type','access-control-allow-credentials':'true','vary':'Origin'});return res.end();}
  ...
  if(u.pathname==='/'&&req.method==='GET'){const s=sessionFromRequest(req);if(!s)return html(res,401,'<!doctype html><meta charset="utf-8"><title>AMS DEV</title><h1>AMS DEV</h1><p>Application session required.</p>');if(clean(s.role).toLowerCase()!=='manager')return html(res,403,'<!doctype html><meta charset="utf-8"><title>AMS DEV</title><h1>Role not available in external portal yet</h1>');return html(res,200,'<!doctype html><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Audit Management System — Manager Portal</title><style>body{font-family:system-ui;margin:0;background:#f4f5f7;color:#111827}header{background:#111827;color:white;padding:16px 24px}main{padding:22px}.meta{font-size:12px;color:#cbd5e1}.cards{display:flex;gap:12px;flex-wrap:wrap;margin:18px 0}.card{background:white;border:1px solid #e5e7eb;border-radius:10px;padding:14px 18px;min-width:130px}.n{font-size:24px;font-weight:700}table{width:100%;border-collapse:collapse;background:white;font-size:13px}th,td{padding:9px;border-bottom:1px solid #e5e7eb;text-align:left}th{background:#f9fafb}.err{color:#b91c1c}.act{padding:5px 8px;margin:2px;border:1px solid #d1d5db;border-radius:6px;background:white;cursor:pointer}.act:disabled{opacity:.45;cursor:wait}</style><header><strong>Manager Portal</strong><div class="meta">'+esc(s.email)+'</div></header><main><div id="status">Loading audit data…</div><div class="cards" id="cards"></div><div style="display:flex;gap:8px;margin:12px 0;flex-wrap:wrap"><button id="openView" style="padding:8px 12px;border:1px solid #2563eb;border-radius:999px;background:#dbeafe">Open audits</button><button id="completedView" style="padding:8px 12px;border:1px solid #d1d5db;border-radius:999px;background:white">Completed</button><input id="q" placeholder="Company, scope or auditor…" style="padding:8px 12px;border:1px solid #d1d5db;border-radius:999px;min-width:280px"><button id="reset" style="padding:8px 12px;border:1px solid #d1d5db;border-radius:999px;background:white">Reset</button></div><table><thead><tr><th>Company</th><th>Region</th><th>Scopes</th><th>Status</th><th>Planning window</th><th>To be planned</th><th>Hours planned</th><th>Auditor</th><th>Actions</th></tr></thead><tbody id="rows"></tbody></table></main><script>fetch("/api/v1/manager/open",{credentials:"same-origin"}).then(function(r){return r.json()}).then(function(x){if(!x.success)throw new Error(x.error||x.message||"Read failed");document.getElementById("status").textContent=x.rows.length+" open audits · "+x.serverMs+" ms server";var c=x.counts||{};document.getElementById("cards").innerHTML=[["Pending planning",c.pendingPlanning],["Pending approval",c.pendingApproval],["Pending acceptance",c.approved],["Pending completion",c.accepted]].map(function(z){return "<div class=card><div>"+z[0]+"</div><div class=n>"+(z[1]||0)+"</div></div>"}).join("");var all=x.rows.slice();function render(rows){document.getElementById("rows").innerHTML=rows.map(function(r){function e(v){var d=document.createElement("div");d.textContent=v==null?"":v;return d.innerHTML}var acts=(r.allowedActions||[]).filter(function(a){return a!=="PLAN"}).map(function(a){return "<button class=act data-id=\""+e(r.auditId)+"\" data-action=\""+e(a.toLowerCase())+"\">"+e(a)+"</button>"}).join("");return "<tr><td>"+e(r.company)+"</td><td>"+e(r.region)+"</td><td>"+e(r.scopesText)+"</td><td>"+e(r.status)+"</td><td>"+e(r.planningWindowText)+"</td><td>"+e(r.requiredHours)+"</td><td>"+e(r.hoursPlanned)+"</td><td>"+e(r.assignedTo)+"</td><td>"+acts+"</td></tr>"}).join("");document.querySelectorAll(".act").forEach(function(b){b.onclick=function(){var action=b.getAttribute("data-action"),id=b.getAttribute("data-id");if(!confirm(action.toUpperCase()+" audit "+id+"?"))return;document.querySelectorAll(".act").forEach(function(x){x.disabled=true});fetch("/api/v1/manager/action",{method:"POST",credentials:"same-origin",headers:{"content-type":"application/json"},body:JSON.stringify({auditId:id,action:action,options:{}})}).then(function(x){return x.json().then(function(j){if(!x.ok||j.success===false||j.ok===false)throw new Error(j.message||j.error||"Action failed");return j})}).then(function(){location.reload()}).catch(function(e){alert(e.message);document.querySelectorAll(".act").forEach(function(x){x.disabled=false})})}});render(all);var q=document.getElementById("q");q.addEventListener("input",function(){var z=q.value.toLowerCase().trim();render(!z?all:all.filter(function(r){return [r.company,r.region,r.scopesText,r.status,r.assignedTo].join(" ").toLowerCase().indexOf(z)>=0}))});document.getElementById("reset").onclick=function(){q.value="";render(all)};document.getElementById("completedView").onclick=function(){fetch("/api/v1/manager/archived",{credentials:"same-origin"}).then(function(r){return r.json()}).then(function(a){all=(a.rows||[]).map(function(r){return Object.assign({planningWindowText:r.completedDate||r.executedOn||"",requiredHours:r.hoursDedicated||r.hoursPlanned||0,hoursPlanned:r.hoursPlanned||0,assignedTo:r.auditor||""},r)});document.getElementById("status").textContent=all.length+" completed audits · "+a.serverMs+" ms server";document.getElementById("cards").innerHTML="";render(all);document.getElementById("completedView").style.background="#dbeafe";document.getElementById("openView").style.background="white"}).catch(function(e){document.getElementById("status").textContent=e.message})};document.getElementById("openView").onclick=function(){location.reload()}}).catch(function(e){var z=document.getElementById("status");z.className="err";z.textContent=e.message});</script>');}
