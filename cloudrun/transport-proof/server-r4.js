...
const BUILD='2026-09-25_AMS_CLOUD_RUN_MANAGER_PORTAL_R20_ARCHIVED_EQUIVALENCE';
...
function managerArchived(logValues,email,scopeValues,companyValues){
  ...
  const h=logValues[0],ci=col(h,['Audit ID','Audit_ID','AuditId']),cs=col(h,['Status']),cc=col(h,['Company']),cl=col(h,['Location']),ca=col(h,['Auditor','Auditor Email','Email']),cd=col(h,['Date planned']),chp=col(h,['Hours planned']),chd=col(h,['Hours dedicated']),ccd=col(h,['Date completed']),cm=col(h,['Manager email']),cu=col(h,['Company UID']),scopeCat=scopeCatalog(scopeValues||[]),companyIndex=new Map();
  ...
  for(let n=1;n<logValues.length;n++){const r=logValues[n],status=(val(r,cs)||'Completed'),sn=status.toUpperCase().replace(/[\\s-]+/g,'_');if(sn!=='COMPLETED')continue;const mgr=val(r,cm).toLowerCase();const company=val(r,cc);if(!company&&!val(r,ci))continue;const location=val(r,cl),uid=val(r,cu),scopes=scopesForAudit({h,row:r},scopeCat);rows.push({auditId:val(r,ci)||('LOGROW_'+(n+1)),source:'Log realized audits',company,location,region:companyIndex.get(key(uid))||companyIndex.get(key(company+'|'+location))||companyIndex.get(key(company))||'',scopes,scopesText:scopes.join(', '),executedOn:dateOnly(r[cd]),auditor:val(r,ca),hoursPlanned:Number(r[chp])||0,hoursDedicated:Number(r[chd])||0,completedDate:dateOnly(r[ccd]),status:'Completed',statusKey:'COMPLETED',managerEmail:mgr,companyUid:uid,readOnly:true});}
