...
const BUILD='2026-09-26_AMS_CLOUD_RUN_MANAGER_PORTAL_R41_EXTERNAL_FOCUSED_PLANNING_2_0';
...
async function readForm(req){let raw='';for await(const chunk of req){raw+=chunk;if(raw.length>16384)throw new Error('REQUEST_TOO_LARGE');}return new URLSearchParams(raw);}
...
function planningHtml(auditId){
  ...
  </main><script>(function(){const auditId=${id};let model=null;const q=s=>document.querySelector(s);function txt(id,v){q(id).textContent=(v===undefined||v===null||v==='')?'—':String(v)}function renderAvailability(){const email=q('#auditor').value,box=q('#availability');box.innerHTML='';if(!email||!model)return;const rows=(((model.data||{}).overlays||{}).availability||{}).byAuditorEmail?.[email]||[];if(!rows.length){box.innerHTML='<div class="muted">No availability rows in planning window.</div>';return;}for(const r of rows){const d=document.createElement('div');d.className='day '+(r.state==='YES'?'yes':r.state==='NO'?'no':'');const slots=(r.slots||[]).map(s=>'<span class="pill">'+[s.start,s.end,s.company||s.auditId].filter(Boolean).join(' · ')+'</span>').join('');d.innerHTML='<div class="d">'+r.date+'</div><div class="s">'+(r.state||r.available||'')+'</div>'+slots;d.onclick=function(){q('#date').value=r.date};box.appendChild(d)}}
  fetch('/api/v1/planning/workspace?auditId='+encodeURIComponent(auditId),{credentials:'same-origin'}).then(async r=>{const x=await r.json();if(!r.ok||x.ok===false)throw new Error(x.error||'Planning read failed');model=x;const a=x.data?.audit||{};txt('#company',a.company);txt('#auditId',a.auditId);txt('#auditStatus',a.status);txt('#window',[a.planningWindowFrom,a.planningWindowTo].filter(Boolean).join(' → '));txt('#scopes',(a.scopes||[]).join(', '));q('#date').min=a.planningWindowFrom||'';q('#date').max=a.planningWindowTo||'';const sel=q('#auditor');for(const c of a.candidateAuditors||[]){const o=document.createElement('option');o.value=c.email;o.textContent=(c.name||c.email)+(c.isPreassigned?' · preassigned':'');o.dataset.blocked=c.blockedWeekdays||'';sel.appendChild(o)}if(a.candidateAuditors?.length){sel.value=a.candidateAuditors[0].email;txt('#auditorMeta',a.candidateAuditors[0].blockedWeekdays?'Default less available: '+a.candidateAuditors[0].blockedWeekdays:'');}q('#timing').textContent=(x.timing?.totalMs||0)+' ms server';q('#status').textContent='Ready';q('#status').className='status ok';renderAvailability();}).catch(e=>{q('#status').textContent=e.message;q('#status').className='status err'});q('#auditor').addEventListener('change',function(){const o=this.options[this.selectedIndex];txt('#auditorMeta',o?.dataset.blocked?'Default less available: '+o.dataset.blocked:'');renderAvailability()});})();</script></body></html>`;
}
