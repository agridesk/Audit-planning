 ...
 * BUILD: AMS01_MANAGER_TOOLKIT_COLD_OPEN_PERF_ZZ_20260908_R2
 ...
 * - If an auditor is present, read only Auditors A:P and resolve blocked weekdays.
 * - Preserve the existing companyConstraints output contract.
 */

var AMS01_MANAGER_TOOLKIT_COLD_OPEN_PERF_BUILD = 'AMS01_MANAGER_TOOLKIT_COLD_OPEN_PERF_ZZ_20260908_R2';
  ...
  if (!auditorEmail && !auditorName) {
    ...
    try {
      ...
      if (sh) {
        ...
        if (lastRow >= 2) {
          var width = Math.min(sh.getLastColumn(), 16); // A:P canonical fields needed here
