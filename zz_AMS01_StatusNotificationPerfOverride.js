 ...
 * BUILD: AMS01_STATUS_NOTIFICATION_PERF_ZZ_20260908_R1
 * DEV-only late-load override to ensure AMS-01 notification performance route
 * wins over canonical duplicate function declarations during validation.
 */
var AMS01_STATUS_NOTIFICATION_PERF_ZZ_BUILD='AMS01_STATUS_NOTIFICATION_PERF_ZZ_20260908_R1';
...
function AMS01_GetCompanyNumberByUid_(ss,companyUid){
  ...
  var map=null;try{var raw=CacheService.getScriptCache().get(AMS01_COMPANY_NUMBER_CACHE_KEY);if(raw)map=JSON.parse(raw);}catch(eCache){map=null;}
  if(!map){map={};try{ss=ss||SpreadsheetApp.getActiveSpreadsheet()||SpreadsheetApp.getActive();var sh=ss&&ss.getSheetByName('Companies');if(!sh)return '';var lastRow=sh.getLastRow(),lastCol=sh.getLastColumn();if(lastRow<2||lastCol<1)return '';var hdr=sh.getRange(1,1,1,lastCol).getDisplayValues()[0]||[],idx=StatusNotificationBridge_HeaderMapLoose_(hdr),uidCol=-1,numCol=-1;['Company_UID','Company UID','CompanyUID','COMPANY_UID','UID'].some(function(k){var nk=String(k).toLowerCase().replace(/[^a-z0-9]/g,'');if(Object.prototype.hasOwnProperty.call(idx,nk)){uidCol=idx[nk];return true;}return false;});['Number','Company Number','Client Number','Customer Number'].some(function(k){var nk=String(k).toLowerCase().replace(/[^a-z0-9]/g,'');if(Object.prototype.hasOwnProperty.call(idx,nk)){numCol=idx[nk];return true;}return false;});if(uidCol<0||numCol<0)return '';var minCol=Math.min(uidCol,numCol)+1,width=Math.abs(numCol-uidCol)+1,vals=sh.getRange(2,minCol,lastRow-1,width).getDisplayValues(),uidOff=uidCol-(minCol-1),numOff=numCol-(minCol-1);for(var i=0;i<vals.length;i++){var u=String(vals[i][uidOff]||'').trim();if(u)map[u]=StatusNotificationBridge_NormalizeMpsNumber_(vals[i][numOff]||'');}try{var json=JSON.stringify(map);if(json.length<90000)CacheService.getScriptCache().put(AMS01_COMPANY_NUMBER_CACHE_KEY,json,900);}catch(ePut){}}catch(eBuild){map={};}}
...
function StatusNotificationBridge_LoadEcasAuditBriefing_(auditId){
  ...
  try{var ss=SpreadsheetApp.getActiveSpreadsheet()||SpreadsheetApp.getActive();if(!ss||typeof __mp_getAuditPlanningRow_!=='function')return out;var pack=__mp_getAuditPlanningRow_(ss,auditId);if(!pack||!pack.row||!pack.hdr)return out;var hdr=pack.hdr||[],row=pack.row||[],idx=StatusNotificationBridge_HeaderMapLoose_(hdr);out.company=StatusNotificationBridge_CellLoose_(row,idx,['Company','Company name','Client','Customer','Organisation','Organization']);out.companyUid=StatusNotificationBridge_CellLoose_(row,idx,['Company_UID','Company UID','CompanyUID','UID','Company Id','Company ID']);out.mpsNumber=AMS01_GetCompanyNumberByUid_(ss,out.companyUid);if(out.mpsNumber)out.__ams01MpsSource='CompactUidNumberIndex';if(!out.mpsNumber){out.mpsNumber=StatusNotificationBridge_ExtractMpsNumberFromAuditRow_(hdr,row,idx,row);if(out.mpsNumber)out.__ams01MpsSource='AuditPlanningRow';}if(!out.mpsNumber){out.mpsNumber=StatusNotificationBridge_LoadMpsNumberFromCompaniesByUid_(ss,out.companyUid);if(out.mpsNumber)out.__ams01MpsSource='CompaniesFallback';}out.planningJson=StatusNotificationBridge_CellLoose_(row,idx,['Planning JSON','PlanningJSON','Planning','Planning json','Planning_Js','Planning js']);out.scopes=StatusNotificationBridge_ExtractScopesFromAuditRow_(hdr,row);var parsed=StatusNotificationBridge_ParsePlanningJson_(out.planningJson);out.blocks=parsed.blocks;out.plannedDates=parsed.plannedDates;out.plannedHours=parsed.plannedHours;out.auditorEmail=parsed.auditorEmail;out.auditorName=parsed.auditorName;if(!out.plannedHours&&out.blocks.length)out.plannedHours=StatusNotificationBridge_SumBlockHours_(out.blocks);try{Logger.log('[AMS01_ACCEPT_BRIEFING] '+JSON.stringify({build:AMS01_STATUS_NOTIFICATION_PERF_ZZ_BUILD,auditId:auditId,row:pack.rowNumber||0,indexFromCache:!!pack.indexFromCache,mpsSource:out.__ams01MpsSource,scopes:out.scopes.length,blocks:out.blocks.length}));}catch(eLog){}}catch(e){try{Logger.log('[AMS01_ACCEPT_BRIEFING_ERROR] '+String(e&&e.message?e.message:e));}catch(eLog2){}}
  return out;
}
